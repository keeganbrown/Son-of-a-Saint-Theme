...
if (!function_exists('bootstrapBasicCategoriesList')) {
	...
	{
		return sprintf('<span class="categories-icon glyphicon glyphicon-th-list" title="' . __('Posted in', 'son-of-a-saint') . '"></span> %1$s', $categories_list);
		...
		if ('pingback' == $comment->comment_type || 'trackback' == $comment->comment_type) { 
			...
			echo '<div class="comment-body media">';
				echo '<div class="media-body">';
					_e('Pingback:', 'son-of-a-saint');
					comment_author_link(); 
					edit_comment_link(__('Edit', 'son-of-a-saint'), '<span class="edit-link">', '</span>');
				echo '</div>';
			echo '</div>';
		} else {
			echo '<li id="comment-';
				...
				echo '<div class="comment-content media-body">';
					echo '<div class="comment-author vcard">';
						...
						echo '<time datetime="';
							comment_time('c');
						echo '">';
						printf(_x('%1$s at %2$s', '1: date, 2: time', 'son-of-a-saint'), get_comment_date(), get_comment_time());
						echo '</time>';
						echo '</a>';
						// end date-time
						...
						edit_comment_link('<span class="fa fa-pencil-square-o "></span>' . __('Edit', 'son-of-a-saint'), '<span class="edit-link">', '</span>');
						...
						if ('0' == $comment->comment_approved) {
							echo '<div class="comment-awaiting-moderation text-warning"> <span class="glyphicon glyphicon-info-sign"></span> ';
								_e('Your comment is awaiting moderation.', 'son-of-a-saint');
							echo '</div>';
						} //endif;

						// comment author says
						printf(__('%s <span class="says">says:</span>', 'son-of-a-saint'), sprintf('<cite class="fn">%s</cite>', get_comment_author_link()));
					...
					comment_reply_link(array_merge($args, array(
						...
						'reply_text' => '<span class="fa fa-reply"></span> ' . __('Reply', 'son-of-a-saint'),
						'login_text' => '<span class="fa fa-reply"></span> ' . __('Log in to Reply', 'son-of-a-saint')
					)));
					// end reply link
...
if (!function_exists('bootstrapBasicEditPostLink')) {
	...
	{
		return edit_post_link('<b class="edit-post-icon glyphicon glyphicon-pencil" title="' . __('Edit', 'son-of-a-saint') . '"></b>', '<span class="edit-post-link btn btn-default btn-xs" title="' . __('Edit', 'son-of-a-saint') . '">', '</span>');
...
if (!function_exists('bootstrapBasicFullPageSearchForm')) {
	...
	function bootstrapBasicFullPageSearchForm() 
	{
		...
		$output .= '<input type="text" name="s" value="' . esc_attr(get_search_query()) . '" placeholder="' . esc_attr_x('Search &hellip;', 'placeholder', 'son-of-a-saint') . '" title="' . esc_attr_x('Search &hellip;', 'label', 'son-of-a-saint') . '" class="form-control" />';
		$output .= '</div>';
		$output .= '<div class="col-xs-2">';
		$output .= '<button type="submit" class="btn btn-default">' . __('Search', 'son-of-a-saint') . '</button>';
		$output .= '</div>';
		$output .= '</div>';
		$output .= '</form>';
...
if (!function_exists('bootstrapBasicMoreLinkText')) {
	...
	{
		return __('Continue reading <span class="meta-nav">&rarr;</span>', 'son-of-a-saint');
...
if (!function_exists('bootstrapBasicPostOn')) {
	...
	function bootstrapBasicPostOn() 
	{
		...
		printf(__('<span class="posted-on">Posted on %1$s</span><span class="byline"> by %2$s</span>', 'son-of-a-saint'),
			...
			sprintf('<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s">%3$s</a></span>',
				...
				esc_attr(sprintf(__('View all posts by %s', 'son-of-a-saint'), get_the_author())),
...
if (!function_exists('bootstrapBasicTagsList')) {
	...
	{
		return sprintf('<span class="tags-icon glyphicon glyphicon-tags" title="' . __('Tagged', 'son-of-a-saint') . '"></span>&nbsp; %1$s', $tags_list);
